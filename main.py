    ...
    if filtro.get("cliente") and not filtro.get("cliente_exato") and 'NOME_CLIENTE' in df.columns:
        ...
        for raiz, grp in _por_raiz.items():
            ...
            if len(nomes_unicos) == 1:
                ...
                # Nomes diferentes com mesma raiz → agrupa em UMA opção (nome mais curto + contagem)
                nome_principal = min(nomes_unicos, key=len)  # nome mais curto = nome raiz/principal
                total_cnpjs = sum(int(
                    _resumo[(_resumo['NOME_CLIENTE'] == n) & (_resumo['_raiz'] == raiz)]['n_cnpjs'].sum()
                ) for n in nomes_unicos)
                filiais_fmt = f" ({total_cnpjs} filiais)" if total_cnpjs > 1 else f" ({len(nomes_unicos)} razões sociais)"
                _opcoes_agrupadas.append({
                    "label": f"{nome_principal}{raiz_fmt}{filiais_fmt}",
                    "label_nome": nome_principal,
                    "cnpj_raiz": raiz if _tem_cnpj else "",
                    "nome_exato": nome_principal,
                    "n_filiais": total_cnpjs,
                })
